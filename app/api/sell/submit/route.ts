...
export async function POST(request: Request) {
  try {
    ...
    if (!session) {
      console.warn("Sell submit: Unauthorized request");
    ...
    try {
      ...
      console.error("Sell submit: JSON parse error:", parseError);
      return NextResponse.json(
        {
    ...
    if (missingFields.length > 0) {
      console.warn("Sell submit: Missing required fields:", missingFields);
      return NextResponse.json(
        {
    ...
    if (
      ...
      console.warn("Sell submit: Invalid identity document URL");
      return NextResponse.json(
        {
    ...
    if (
      ...
      console.warn("Sell submit: Invalid home map URL");
      return NextResponse.json(
        {
    ...
    try {
      ...
      if (!tableCheck) {
        console.error("Sell submit: Table 'sell_submission' does not exist");
        return NextResponse.json(
          {
            error: "Database configuration error",
            details:
              process.env.NODE_ENV === "development"
                ? "Table 'sell_submission' not found. Please run migrations."
                : undefined,
          },
          { status: 500 }
        );
      ...
      console.error("Sell submit: Database table check error:", tableError);
      return NextResponse.json(
        {
    ...
    try {
      ...
      console.error("Sell submit: Database insert error:", {
    ...
    console.log("Sell submit: Successfully created submission", {
      id,
      userId: session.userId,
    });
    ...
    console.error("Sell submit: Unexpected error:", {
